...
const BookCard = ({ book }) => {
    const { id, name, author, img, tags, category, rating } = book;
        ...
        <Link to={`book-details/${id}`}>
            <div className="p-6 rounded-xl drop-shadow-lg dark:bg-gray-50 dark:text-gray-900 cursor-pointer">
                <div className="flex justify-center bg-slate-100 p-4">
                    <img src={img} alt="" className="rounded-md h-72 dark:bg-gray-500" />
                </div>
                <div className="flex gap-4 mt-6 mb-2">
                    ...
                    {/* <span className="block text-xs font-medium tracking-widest uppercase dark:text-violet-600 border py-2 px-3 rounded-full">Quisque</span>
                    <span className="block text-xs font-medium tracking-widest uppercase dark:text-violet-600 border py-2 px-3 rounded-full">Quisque</span> */}
                </div>
                <h2 className="text-2xl font-semibold tracking-wide my-4">{name}</h2>
